...
class seq2seqModel(object):
    ...
    def __init__(self,lstm_units,embeddings,go,train=True,train_embeddings=False,bidirectional=True, condition=None):
        """
        ...
        #sentence and parameters
        self.sentence=tf.placeholder(tf.int32,[None,None],'sentence')
        self.sentence_size=tf.placeholder(tf.int32,[None],'senetence_size')
        ...
        self.decoder_step_input=tf.placeholder(tf.int32,[None],'prediction_step')

        self.decoder_fw_step_c = tf.placeholder(tf.float32,
                                                [None, lstm_units], 'decoder_fw_step_state_c')
        self.decoder_fw_step_h = tf.placeholder(tf.float32,
                                                [None, lstm_units], 'decoder_fw_step_state_h')
        self.decoder_bw_step_c = tf.placeholder(tf.float32,
                                                [None, lstm_units],'decoder_bw_step_state_c')
        self.decoder_bw_step_h = tf.placeholder(tf.float32,
                                                [None, lstm_units],'decoder_bw_step_state_h')

        with tf.variable_scope('seq2seq') as self.scope:
            if condition!= None:
                self.conditions=tf.Variable(tf.random_normal([100]),name='style_embedding',trainable=True)
            self.embeddings=tf.Variable(embeddings,name='embeddings',trainable=train_embeddings)
            ...
            #encoding step
            if bidirectional:
                ret=tf.nn.bidirectional_dynamic_rnn(self.lstm_fw,self.lstm_bw,embedded,dtype=tf.float32,
                                                    sequence_length=self.sentence_size,scope=self.scope)
            else:
                ret=tf.nn.dynamic_rnn(self.lstm_fw,embedded,dtype=tf.float32,
                                      sequence_length=self.sentence_size, scope=self.scope)
            ...
            if bidirectional:
                if condition!=None:
                    decoder_input=tf.concat([decoder_input,self.conditions],axis=0)
                outputs, _ = tf.nn.bidirectional_dynamic_rnn(self.lstm_fw, self.lstm_bw, decoder_input,
                                                             self.sentence_size, encoded_state_fw, encoded_state_bw)
                # concat fw and bw outputs
                outputs = tf.concat(outputs, -1)
            else:
                if condition!=None:
                    decoder_input=tf.concat([decoder_input,condition])
                outputs, _ = tf.nn.dynamic_rnn(self.lstm_fw, decoder_input,
                                               self.sentence_size,encoded_state_fw)
        ...
        embedded_step = tf.nn.embedding_lookup(self.embeddings,self.decoder_step_input)
        state_fw = tf.nn.rnn_cell.LSTMStateTuple(self.decoder_fw_step_c,self.decoder_fw_step_h)
        state_bw = tf.nn.rnn_cell.LSTMStateTuple(self.decoder_bw_step_c, self.decoder_bw_step_h)
        ...
        if bidirectional:
            with tf.variable_scope(bw_scope_name, reuse=True):
                ...
                step_output = tf.concat(axis=1, values=[step_output_fw,step_output_bw])
        else:
            step_output = step_output_fw

        with tf.variable_scope(self.projection_scope, reuse=True):
            self.projected_step_output = tf.layers.dense(step_output,self.vocab_size)
    ...
    def _create_training_tensors(self):
        ...
        sampled_loss = tf.nn.sampled_softmax_loss(projection_w_t, projection_b, labels,
                                                  decoder_outputs2d, 100,self.vocab_size)
    ...
    def train(self,session,save_path,train_data,valid_data,batch_size,epochs,learning_rate,
              dropout_keep,clip_value,report_interval):
        """
        Train the model
    ...
    def load(cls, directory, session, train=False):
        ...
        ae = seq2seqModel(metadata['num_units'], dummy_embeddings,metadata['go'], train=train,
